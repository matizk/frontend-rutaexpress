...
export class AdminPageComponent implements OnInit {
  ...
  protected create(): void { if (this.form.invalid) { this.form.markAllAsTouched(); return; } this.api.create(this.form.getRawValue()).subscribe({ next: (shipment) => { this.shipments.update((items) => [shipment, ...items]); this.form.reset({ direccionOrigen: 'Santiago', pesoKg: 1 }); }, error: () => this.error.set('El BFF rechazó la creación. Comprueba que exista un JWT de Entra válido.') }); }
